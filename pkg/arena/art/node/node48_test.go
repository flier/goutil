...
func TestNode48_Shrink(t *testing.T) {
	Convey("Given a Node48", t, func() {
		...
		Convey("When shrinking with exactly 11 children", func() {
			// Add 11 children
			for i := 0; i < 11; i++ {
				child := NewLeaf[any](a, []byte{byte(i * 20)}, nil)
				node.AddChild(byte(i*20), child)
			}
